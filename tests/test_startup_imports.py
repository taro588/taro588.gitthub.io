...
def test_core_imports_without_dcc():
    code = "import src.core.asset, src.core.task, src.core.pipeline; import src.core.registry, src.core.validation; print('ok')"
    out = run_python(code)
...
def test_launcher_import_without_dcc():
    ...
    out = run_python(code)
    assert out.returncode == 0, out.stderr
    assert "degraded" in out.stdout or "ready" in out.stdout
