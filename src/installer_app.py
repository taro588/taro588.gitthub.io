...
def bundled_src():
    return Path(getattr(sys,"_MEIPASS",Path(__file__).resolve().parents[1]))/"src"
...
class InstallerApp:
    def __init__(self):
        self.root=tk.Tk(); self.root.title("GameArt AI Toolkit Installer"); self.root.geometry("900x700"); self.root.minsize(820,620)
    ...
    def _build(self):
        ...
        ttk.Label(o,text="一键安装 · 无需 Python").pack(anchor="w",pady=(2,14))
        row=ttk.Frame(o); row.pack(fill="x")
        ttk.Label(row,text="安装位置：").pack(side="left")
        self.path_var=tk.StringVar(value=str(self.install_root)); ttk.Entry(row,textvariable=self.path_var).pack(side="left",fill="x",expand=True,padx=8)
        ttk.Button(row,text="浏览…",command=self.choose_path).pack(side="right")
        hosts=detect_hosts(); self.host_vars={}
        hf=ttk.LabelFrame(o,text="主机集成"); hf.pack(fill="x",pady=14)
        for i,(h,label) in enumerate((("maya","Maya"),("3ds_max","3ds Max"))):
            v=tk.BooleanVar(value=hosts[h]); self.host_vars[h]=v
            ttk.Checkbutton(hf,text=f"{label}（自动检测：{'已检测到' if hosts[h] else '未检测到'}）",variable=v).grid(row=0,column=i,sticky="w",padx=12,pady=8)
        ttk.Label(o,text="第三方插件（可跳过）：").pack(anchor="w")
        self.vars={}; g=ttk.Frame(o); g.pack(fill="x",pady=4)
        ...
        b=ttk.Frame(o); b.pack(fill="x",pady=16)
        self.btn=ttk.Button(b,text="一键安装",command=self.start); self.btn.pack(side="left")
        ttk.Button(b,text="检查环境",command=lambda:self._run(self.doctor)).pack(side="left",padx=8)
        ttk.Button(b,text="退出",command=self.root.destroy).pack(side="right")
        self.progress=ttk.Progressbar(o,mode="indeterminate"); self.progress.pack(fill="x")
        ttk.Label(o,textvariable=self.status).pack(fill="x",pady=8)
    ...
    def _run(self,fn):
        self.status.set("处理中…"); self.progress.start(12); threading.Thread(target=self._worker,args=(fn,),daemon=True).start()
    def _worker(self,fn):
        try:r=fn()
    ...
    def start(self):
        self.install_root=Path(self.path_var.get()).expanduser().resolve(); self.btn.configure(state="disabled"); self._run(self.install_all)
    def install_all(self):
        root=self.install_root; root.mkdir(parents=True,exist_ok=True); payload=root/"current"; payload.mkdir(parents=True,exist_ok=True)
        src=bundled_src(); dst=payload/"src"
        ...
        for n,v in self.vars.items():
            if v.get():
                self.write(f"安装插件：{n}"); results.append(pi.install(n).__dict__)
    ...
    def doctor(self): return {"ok":True,"install_root":str(self.install_root),"hosts":detect_hosts()}
